...
{  
    ...
    public class MasterController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> AddCity([FromBody] AddCityCommand request)
        {
        ...
        #endregion
        #region Created by Rohit
        public IActionResult PortMaster()
        {
            return View();
        }
        [HttpPost]
        public async Task<IActionResult> AddPort([FromBody] AddPortCommand request)
        {
            request.CreatedBy = Convert.ToInt32(User.FindFirst(ClaimTypes.NameIdentifier)?.Value);
            var result=await _mediator.Send(request);
            return Json(result);
        ...
        public async Task<IActionResult> GetPort() {
            var response = await _mediator.Send( new GetPortListQuery());
            return Json(response);
        }
        #endregion
